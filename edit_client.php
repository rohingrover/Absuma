...
try {
    // Fetch client details with rates
    $stmt = $pdo->prepare("
        SELECT * FROM clients 
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_client'])) {
        try {
            ...
            // Handle rates update - Delete existing and add new ones
            if (isset($_POST['update_rates']) && $_POST['update_rates'] == '1') {
                    ...
                    foreach ($_POST['movement_types_20ft'] as $index => $movement_type) {
                        if (!empty($movement_type) && !empty($_POST['rates_20ft'][$index])) {
                            ...
                            $rateStmt = $pdo->prepare("
                                ...
                                VALUES (?, '20ft', ?, ?, ?, ?, ?, NOW())
                            ");
                            $rateStmt->execute([$clientId, $movement_type, $container_type, $import_export, $rate, $remarks]);
                        }
                    }
                }
    ...
    // Get existing client rates for display
...
<body class="bg-gray-50">
                ...
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-blue-600 text-sm">Edit Client - <?= htmlspecialchars($client['client_code'] ?? 'Unknown') ?></p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <main class="flex-1">
                    ...
                    <form method="POST" class="space-y-6">
                                ...
                                <div class="form-grid">
                                    <div class="form-field">
                                        ...
                                        <input type="number" name="billing_cycle_days" id="billing_cycle_days" required min="1" max="365"
                                               value="<?= htmlspecialchars($client['billing_cycle_days']) ?>"
                                               class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                               placeholder="e.g. 30">
                            ...
                            <div class="p-6">
                                <div class="flex justify-between items-center mb-6">
                                    <h2 class="text-lg font-semibold text-gray-800 flex items-center gap-2">
                                        <i class="fas fa-money-bill text-green-600"></i> Current Rates
                                    </h2>
                                    <button type="button" id="editRatesBtn" class="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                        <i class="fas fa-edit mr-2"></i> Edit Rates
                                    </button>
                                </div>
                                
                                <!-- Display current rates in a compact format -->
                                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    <?php
                                    $rates20ft = array_filter($client_rates, function($rate) { return $rate['container_size'] === '20ft'; });
                                    $rates40ft = array_filter($client_rates, function($rate) { return $rate['container_size'] === '40ft'; });
                                    ?>
                                    
                                    <?php if (!empty($rates20ft)): ?>
                                    <div>
                                        ...
                                        <div class="space-y-2">
                                            <?php foreach ($rates20ft as $rate): ?>
                                            <div class="bg-gray-50 p-3 rounded text-sm">
                                                <div class="font-medium"><?= htmlspecialchars($rate['movement_type']) ?></div>
                                                <div class="text-gray-600">
                                                    <?= htmlspecialchars($rate['container_type']) ?> | 
                                                    <?= htmlspecialchars($rate['import_export']) ?> | 
                                                    <span class="font-medium text-green-600">₹<?= number_format($rate['rate'], 2) ?></span>
                                                </div>
                                                <?php if ($rate['remarks']): ?>
                                                <div class="text-xs text-gray-500 mt-1"><?= htmlspecialchars($rate['remarks']) ?></div>
                                                <?php endif; ?>
                                            </div>
                                            <?php endforeach; ?>
                                        </div>
                        ...
                        <!-- Edit Rates Section (Hidden by default) -->
                        <div id="editRatesSection" class="space-y-6" style="display: none;">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const editRatesSection = document.getElementById('editRatesSection');
            
            if (editRatesBtn) {
                editRatesBtn.addEventListener('click', function() {
                    editRatesSection.style.display = editRatesSection.style.display === 'none' ? 'block' : 'none';
                    this.textContent = editRatesSection.style.display === 'none' ? 'Edit Rates' : 'Cancel Edit';
                    
                    if (editRatesSection.style.display === 'block') {
                        // Initialize rate editing
        ...
        function loadExistingRates() {
            // Load existing rates for editing
            const clientRates = <?= json_encode($client_rates) ?>;
            
            clientRates.forEach(rate => {
                if (rate.container_size === '20ft') {
                    addRate20ft();
                    const lastRow = document.querySelector('#rates20ftContainer .rate-row:last-child');
                    fillRateRow(lastRow, rate);
                } else if (rate.container_size === '40ft') {
                    addRate40ft();
                    const lastRow = document.querySelector('#rates40ftContainer .rate-row:last-child');
                    fillRateRow(lastRow, rate);
                }
            });
        }

        function fillRateRow(row, rate) {
            const selects = row.querySelectorAll('select');
            const inputs = row.querySelectorAll('input');
            
            // Fill movement type
            selects[0].value = rate.movement_type;
            // Fill container type
            selects[1].value = rate.container_type;
            // Fill import/export
            selects[2].value = rate.import_export;
            // Fill rate
            inputs[0].value = rate.rate;
            // Fill remarks
            inputs[1].value = rate.remarks || '';
        }
