...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        $sql = "INSERT INTO vendors (
            company_name, registered_address, invoice_address, contact_person,
            mobile, email, constitution_type, business_nature, bank_name, 
            account_number, branch_name, ifsc_code, pan_number, gst_number, 
            state, state_code, vendor_type, total_vehicles, status, created_by, created_at
        ) VALUES (
            :company_name, :registered_address, :invoice_address, :contact_person,
            :mobile, :email, :constitution_type, :business_nature, :bank_name,
            :account_number, :branch_name, :ifsc_code, :pan_number, :gst_number,
            :state, :state_code, :vendor_type, :total_vehicles, 'pending', :created_by, :created_at
        )";
        ...
        $vendor_code = 'VND' . str_pad($vendor_id, 6, '0', STR_PAD_LEFT);
        $pdo->prepare("UPDATE vendors SET vendor_code = ? WHERE id = ?")
             ->execute([$vendor_code, $vendor_id]);
        ...
        foreach ($documents as $field => $doc_type) {
            if (isset($_FILES[$field]) && $_FILES[$field]['error'] == 0) {
                ...
                if (in_array($file_extension, $allowed_extensions)) {
                    $file_name = $vendor_code . '_' . $field . '_' . time() . '.' . $file_extension;
...
<head>
    ...
    <title>Vendor Registration - Fleet Management</title>
...
<body class="bg-gray-50">
                ...
                <div class="flex justify-between items-center">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-blue-600 text-sm">Vendor Registration</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-4">
                ...
                <aside class="w-full lg:w-72 flex-shrink-0">
                    <div class="bg-white rounded-xl shadow-sm p-4 sticky top-20">
                        <div class="mb-6">
                            ...
                            <nav class="space-y-1">
                                <a href="vendor_registration.php" class="flex items-center gap-2 px-3 py-2 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg">
                                    <i class="fas fa-plus w-5 text-center"></i> Register Vendor
                                </a>
                                <a href="manage_vendors.php" class="flex items-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-all">
                ...
                <main class="flex-1">
                   ...
                   <form method="POST" enctype="multipart/form-data" class="space-y-6">
                                ...
                                <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 mb-6 rounded-lg">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm">Please add all commercial vehicles that will be operating under our services. These vehicles will be tracked separately for billing and service purposes.</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="p-6">
                                ...
                                <div class="form-field">
                                    ...
                                    <select name="business_nature" id="business_nature" required 
                                        ...
                                        <option value="Transport Contractor">Transport Contractor</option>
                                ...
                                <div class="form-grid">
                                    ...
                                    <div class="form-field">
                                        ...
                                        <select name="state" id="state_select" required 
                                            ...
                                            <option value="Andhra Pradesh" data-code="AP">Andhra Pradesh</option>
                                            <option value="Bihar" data-code="BR">Bihar</option>
                                            <option value="Delhi" data-code="DL">Delhi</option>
                                            <option value="Gujarat" data-code="GJ">Gujarat</option>
                                            <option value="Haryana" data-code="HR">Haryana</option>
                                            <option value="Karnataka" data-code="KA">Karnataka</option>
                                            <option value="Maharashtra" data-code="MH">Maharashtra</option>
                                            <option value="Punjab" data-code="PB">Punjab</option>
                                            <option value="Rajasthan" data-code="RJ">Rajasthan</option>
                                            <option value="Tamil Nadu" data-code="TN">Tamil Nadu</option>
                                            <option value="Uttar Pradesh" data-code="UP">Uttar Pradesh</option>
                                            <option value="West Bengal" data-code="WB">West Bengal</option>
                                        </select>
                        ...
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6 card-hover-effect">
                            <div class="p-6">
                                <h2 class="text-lg font-semibold text-gray-800 mb-6 flex items-center gap-2">
                                    <i class="fas fa-paperclip text-blue-600"></i> Document Upload <span class="text-sm text-gray-500 font-normal">(Optional)</span>
                                </h2>
                                ...
                                <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 mb-6 rounded-lg">
                                    <div class="flex items-center gap-2">
                                        <i class="fas fa-info-circle text-blue-600"></i>
                                        <div>
                                            <p class="font-medium">Document Guidelines</p>
                                            <p class="text-sm">You can upload documents now or later. Accepted formats: JPG, PNG, PDF (Max 5MB each)</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6 card-hover-effect">
                            <div class="p-6">
                                <h2 class="text-lg font-semibold text-gray-800 mb-6 flex items-center gap-2">
                                    <i class="fas fa-check-circle text-blue-600"></i> Declaration
                                </h2>
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    <div class="flex items-start gap-3">
                                        <input type="checkbox" id="declaration" required class="mt-1 w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                                        <label for="declaration" class="text-sm text-gray-700">
                                            I hereby declare that the above information is true in all respects and I undertake to inform 
                                            if any changes in the above particulars regarding our business from time to time. I also confirm 
                                            that all vehicle numbers provided are owned/operated by our company and are authorized for commercial use.
                                        </label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                <div class="flex items-center gap-3 justify-end">
                                    ...
                                    <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                        <i class="fas fa-save mr-2"></i> Register Vendor
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        let vehicleCount = <?= count($formData['vehicle_numbers']) ?>;
